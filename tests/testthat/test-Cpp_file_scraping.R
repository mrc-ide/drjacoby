test_that("cpp file scraping  works", {
  Rcpp::sourceCpp("test_input_files/sourced_likelihood.cpp")
  expect_equal(loglike(c(mu = 0, sigma = 2), list(x = c(0, 0, -1, 1)), NULL),  -6.698343, tol = 0.00001)
  
  t1 <- cpp_function_get("test_input_files/sourced_likelihood.cpp")
  expect_null(check_likelihood_compilation(t1))
