...
for(i in use){
  ...
  if(i == "cpp11"){
    ...
    ll <- loglike_normal_cpp11
    lp <- logprior_null_cpp11
  }
  ...
  if(i == "rcpp"){
    ...
    ll <- loglike_normal_rcpp
    lp <- logprior_null_rcpp
  }
  
  test_that("Chains = 1 Rungs = No", {
    set.seed(1)
    mcmc <- dj$new( 
      data = data,
      df_params = df_params,
  ...
  test_that("Chains = 1 Rungs = Yes", {
    ...
    # MC acceptance rate
    mc_ar <- mcmc$mc_acceptance_rate(phase = c("tune", "burn", "sample"))
  ...
  test_that("Chains > 1 Rungs = Yes", {
    ...
    mcmc$set_beta(beta = seq(1, 0, length.out = 3))
    mcmc$burn(iterations = iterations, silent = TRUE)
    mcmc$burn(iterations = iterations, silent = TRUE)
    mcmc$sample(iterations = iterations, silent = TRUE)
    mcmc$sample(iterations = iterations, silent = TRUE)
    ...
    # MC acceptance rate
    mc_ar <- mcmc$mc_acceptance_rate(c("burn", "sample"))
    expect_type(mc_ar, "list")
    expect_equal(dim(mc_ar), c(4, 3))
    expect_named(mc_ar, c("phase", "beta_mid", "coupling_acceptance"))
    expect_equal(sum(is.na(mc_ar)), 0)
    
    mc_ar <- mcmc$mc_acceptance_rate(phase = "burn")
    expect_type(mc_ar, "list")
    expect_equal(dim(mc_ar), c(2, 3))
    expect_named(mc_ar, c("phase", "beta_mid", "coupling_acceptance"))
    expect_equal(sum(is.na(mc_ar)), 0)
