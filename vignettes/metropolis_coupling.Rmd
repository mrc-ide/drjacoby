...
We now find that with just 20 rungs we have decent acceptance rates all the way through the temperature ladder, and can therefore be condifent that the prior is "talking to" the posterior. When this is the case it is *unlikely* (but not impossible) that your MCMC is still missing large parts of the posterior distribution. Incidentally, the values of `rungs = 20` and `GTI_pow = 3.0` used here are the default values.
