...
Parallel tempering is perfectly designed to solve this kind of problem. To run parallel tempering
we set up a number of rungs. Each rung represents a different MCMC chain arranged in a "temperature ladder". Hotter chains being more free to explore the parameter space as the likelihood space has
been "melted", making exploration easier. Parameter values can be swapped between rungs in a similar way as newly proposed parameters are accepted or rejected within an MCMC chain. This allows the freely-moving hot chains to propose values to the more constrained colder chains.

To activate parallel tempering, before running the burn-in or sampling we need to run a tuning phase (`$tune()`) to select the number and temperature for our rungs. When looking at the output, we tend to focus exclusively on the coldest chain which can be interpreted the same way as a single chain from regular MCMC.
...
mcmc$tune(target_rung_acceptance = 0.5, n_rungs = 50, iterations = 2000, silent = TRUE)
...
The `$tune()` function works by selecting the number and temperature of rungs against a target rate. It first runs the MCMC for a short period with a reasonably high number of rungs. We can use the output from this phase to inform us how hard it is to traverse from the coldest
...
You can read this just like an elevation plot for a hike! Higher peaks are harder to traverse and indicate we may need to focus more rungs at those temperatures so that they can more easily communicate with each other. Using this, drjacoby automatically picks the number and temperatures of the rungs for the burn-in and sampling phases. We can view the acceptance rates of these rungs to check they are close to the target rung acceptance rate specified in the tuning phase (in this case it was 0.5):

```{r, fig.width=5, fig.height=4}
mcmc$plot_mc_acceptance_rate(beta_axis = FALSE)
```
...
In some cases you may already know the optimal rung schedule. In this instance you can skip the
costly `$tune()` phase and instead, before running `$burn()` call `$set_beta()`.
