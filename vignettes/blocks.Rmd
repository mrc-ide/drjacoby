...
<img src="https://raw.githubusercontent.com/mrc-ide/drjacoby/master/man/figures/dependence_table.png" width="400px" />
...
df_params <- data.frame() |>
  add_parameter(name = "mu1", min = 0, max = Inf, blocks  = c(1, 7)) |>
  add_parameter(name = "mu2", min = 0, max = Inf, blocks = c(2, 7)) |>
  add_parameter(name = "mu3", min = 0, max = Inf, blocks = c(3, 7)) |>
  add_parameter(name = "mu4", min = 0, max = Inf, blocks = c(4, 7)) |>
  add_parameter(name = "mu5", min = 0, max = Inf, blocks = c(5, 7)) |>
  add_parameter(name = "mu6", min = 0, max = Inf, blocks = c(6, 7)) |>
  add_parameter(name = "sigma", min = 0, max = Inf, blocks = 1:6) |>
  add_parameter(name = "phi", min = -Inf, max = Inf, blocks = 7) |>
  add_parameter(name = "tau", min = 0, max = Inf, blocks = 7)
...
likelihood_filename <- system.file("extdata", "blocks_cpp_functions.cpp", package = 'drjacoby', mustWork = TRUE)
...
mcmc <- dj$new(
  data = data_list,
  df_params = df_params,
  loglike = block_loglike_cpp11,
  logprior = block_logprior_cpp11,
  chains = 5
)
...
Assuming we are happy with MCMC performance (convergence, ESS etc.), we can explore the posterior credible intervals of the mu parameters
and compare them to the group-means:

```{r, width = 10, height = 5}
mus <- paste0("mu", 1:6)
data_summary <- data.frame(
  y = mus,
  x = sapply(data_list, mean)
)
mcmc$plot_credible(pars = c("phi", mus)) +
  geom_point(data = data_summary, aes(x = y, y = x), col = "goldenrod", size = 1.5)
```
