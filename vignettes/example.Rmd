...
Next, we need a likelihood function. This **must** have the following input arguments, specified in this order:
...
To use C++ functions within *drjacoby* we simply write them directly within R as character strings. As before, there are some constraints on what this function must look like. It **must** take the same three input arguments described above, defined as follows:
...
We can also define the prior as a C++ function. The `r_logprior()` function defined above can be re-written in C++ as follows:

```{r}
# define cpp logprior function
cpp_logprior <- "SEXP logprior(Rcpp::NumericVector params, Rcpp::List misc) {
