...
Next, we need a likelihood function. This **must** have the following four input arguments, in this order:

1. `params` - a named vector of parameter values
2. `param_i` - an index for the current parameter being updated
3. `data` - a named list of data
4. `misc` - a named list of miscellaneous values

Not all of these inputs will necessarily be used, for example we might have no need for the parameter index or any miscellaneous values, but the `params` and `data` objects will usually be vital. Finally, the function **must** return a single value for the likelihood **in log space**. These constraints on the format of the likelihood function might seem a bit restrictive, but they are needed in order for *drjacoby* to know how to use the function internally. The issue of taking logs is particularly important, as the MCMC will still run even if we forget to take logs, but the results produced will be nonsense!
...
Values close to 1 indicate that the MCMC has converged (typically the threshold <1.1 is used). This output is only availble when running multiple chains.
...
Here we can see that the MCMC continues to move freely after the burn-in phase, and that all chains appear to be exploring the same part of the parameter space. The final plot shows the autocorrelation of the chains, which in this case falls off fairly rapidly with samples being approximately independent at around 5 lags. This is an indication that the MCMC is mixing well. The marginal histogram shows a single clear peak, although this is still a bit rough so we may want to re-run the MCMC with a larger number of sampling iterations to get a smoother result. We can also explore this by looking at the effective sample size (ESS), which is stored within the MCMC diagnostics:
...
To use C++ functions within *drjacoby* we simply write them directly within R as character strings. As before, there are some constraints on what this function must look like. It **must** take the same four input arguments described above, defined in the following format:

1. `params` must be an `Rcpp::NumericVector`
2. `params_i` must be an `int`
3. `data` must be an `Rcpp::List`
4. `misc` must be an `Rcpp::List`

All numeric values will be coerced to `double` within the code, hence data consisting of integer or boolean values should be dealt with as though they are continuous values (for example `TRUE = 1.0`, `FALSE = 0.0`). Second, the function **must** return an object of class `SEXP`. The easiest way to achieve this is to calculate the raw return value as a double, and to use the `Rcpp::wrap()` function to transform to `SEXP`. As before, the value returned should be the likelihood evaluated in **log space**.

Even though we are workin in C++, we still have access to most of R's nice distribution functions through the `R::` namespace. For example, the `dnorm()` function can be accessed within C++ using `R::dnorm()`. A list of probability distributions available within Rcpp can be found [here](https://teuder.github.io/rcpp4everyone_en/220_dpqr_functions.html#gamma-distribution). The `r_loglike()` function defined above can be re-written in C++ as follows:
...
Similarly, we can define the prior as a C++ function, subject to the same constraints. The `r_logprior()` function defined above can be re-written in C++ as follows:
