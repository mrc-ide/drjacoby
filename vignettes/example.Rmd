...
The logic of this function is identical to the R version above, just written in the language of c++. To allow drjacoby to talk to your c++ function(s) we need to have a linking function in this c++ file, in this case called `create_xptr()`. With this set up all we need to do is source our c++ file with `Rccp::sourcecpp()` then simply pass the function names as character strings to `run_mcmc()`. We define the prior in this file in the same way. To make
life easy when setting up a new c++ model, you can create a template for your cpp file with the `cpp_template()` function - then all you need to do is 
fill in the internals of the loglikehood and logprior.
