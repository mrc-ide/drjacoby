...
The logic of this function is identical to the R version above, just written in the language of C++. To allow drjacoby to talk to your c++ function(s) we need to have a linking function in this c++ file, in this case called `create_xptr_loglike()`. With this et up all wee need to do is source our c++ file with `Rccp::sourcecpp()` then simply pass the function names as character strings to `run_mcmc()`. We define the prior in thisa file in the same way.
...
```{r, echo = FALSE, comment = ''}
cpp_funcs <- system.file("extdata/", "example_loglike_logprior.cpp", package = 'drjacoby', mustWork = TRUE)
